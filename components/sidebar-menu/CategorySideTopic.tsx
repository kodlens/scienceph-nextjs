...
const CategorySideTopic = ({ query, category, topic }: Props) => {
  ...
  const loadTopicCounts = async (): Promise<void> => {
    ...
    try {
      ...
      setLoading(true);
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/category-side-menu/topic-labels?${params}`, {
        headers: {
...
export default CategorySideTopic

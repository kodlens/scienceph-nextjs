...
const RelevantArticles = ({ slug, category, topic }: Props) => {

  const [data, setData] = useState<MaterialProps[]>([]);
  ...
  return (
    <div className="space-y-3">
      { data?.map((item: MaterialProps, index) => (
        <Link
          prefetch={false}
          href={`/articles/${item.slug}?category=${item.category_slug}&topic=${topic}`}
          key={`${item.slug}-${index}`}
        ...
        >
          ...
          <p className="mt-1 text-xs text-[#6a7f97]">{dateFormatter(item.publish_date)}</p>
        </Link>
      ))}
    </div>
  )

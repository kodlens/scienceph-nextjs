...
async function getMaterial(param: string, page: number) {
  const search = new URLSearchParams();
  search.set("s", param);
  search.set("page", String(page));

  const res = await fetchFromLaravel<laravelResponse<Material>>(
    `search-latest?${search.toString()}`,
    300
  );
  return res;
}
...
export default async function Search({
  ...
  searchParams: Promise<{ searchParams?: string; page?: string }>;
}) {
  const params = await searchParams;

  console.log(params);
  const qvalue = (params.searchParams || "").trim();

  const rawPage = Number(params.page || "1");
  const currentQueryPage = Number.isFinite(rawPage) && rawPage > 0 ? Math.floor(rawPage) : 1;
  const hasQuery = qvalue.length > 0;

  const materials = hasQuery ? await getMaterial(qvalue, currentQueryPage) : null;
  ...
  return (
    ...
    <main className="min-h-screen bg-[#edf2f6] flex flex-col items-center py-6">
      ...
      {/* Search Container */}
      <div className="w-full lg:max-w-6xl px-4">
        {/* search  */}
